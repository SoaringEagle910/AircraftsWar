    ...
    public List<BaseBullet> ShootAction(AbstractAircraft aircraft) {
        ...
        if(aircraft.getClass()== HeroAircraft.class)
        {
            for(int i=0; i<aircraft.getShootNum(); i++){
                // 子弹发射位置相对飞机位置向前偏移
                // 多个子弹横向分散
                bullet = new HeroBullet(x + (i*2 - aircraft.getShootNum() + 1)*10, y, speedX+(i-aircraft.getShootNum()/2)*4, speedY*2, aircraft.getPower());
                res.add(bullet);
            }
        }
        else
        {
            for(int i=0; i<aircraft.getShootNum(); i++){
                // 子弹发射位置相对飞机位置向前偏移
                // 多个子弹横向分散
                bullet = new EnemyBullet(x + (i*2 - aircraft.getShootNum() + 1)*10, y, speedX+(i-aircraft.getShootNum()/2)*4, speedY, aircraft.getPower());
                res.add(bullet);
            }
        }
